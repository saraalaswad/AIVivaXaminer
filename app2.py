...
def generate_viva_csv(chat_history):
    output = io.StringIO()
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.examiner_logged_in:
            ...
            # ---------- CSV ----------
            if st.button("Generate CSV Transcript"):
                csv_data = generate_viva_csv(st.session_state.messages)
        
                st.download_button(
                    "⬇️ Download CSV",
                    csv_data.getvalue(),
                    file_name="AIViva_Transcript.csv",
                    mime="text/csv"
                )
            else:
